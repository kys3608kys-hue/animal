function doPost(e) {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(["기록일시", "참가자", "조건(글자 수)", "제시 자극", "정답 반응", "참가자 반응", "정오 여부", "반응시간(RT)"]);
  }
  ...
  if (trialsStr) {
    try {
      ...
      for (var i = 0; i < trials.length; i++) {
        var t = trials[i];
        sheet.appendRow([date, name, t.condition || "없음", t.stimulus, t.expected, t.response, t.isCorrect, t.rt]);
      }
    } catch(err) {
